...
# Есть задачи вида:
# Дана коллекция чего-то, необходимо отобрать подходище элементы или величины, посчитанные из элементов
# Например, дан список чисел, отобрать чётные числа

l = [1, 5, 2, 6, 7, 10]

# Для ответа создаётся отдельный список,
# куда по мере перебора элементов исходной коллекции будут добавляться ответы
answers = []

# Перебираем элементы
for i in range(len(l)):
    # Какое-то фильтрующее условие для текущего i-го элемента
    if l[i] % 2 == 0:
        # Подходящие числа добавляется в ответы
        answers.append(l[i])
print('Чётные числа', answers)

# Под такой тип задач подпадает 15 номер





# Иногда бывает, что у нас имеет несколько вложенных циклов,
# и нам в какой-то момент надо выбраться наружу, однако break может вытащить нас
# лишь из текущего цикла, но не из вышестоящего.
# В таком случае применяются ухищрения вроде логических флажков.

# Например:
# Есть 2 списка с числами. Необходимо найти такую пару чисел, которая даст в сумме 100
l1 = [3, 50, 37, 25]
l2 = [63, 49, 3, 20, 77]

# Перебираются все пары чисел
for i in range(len(l1)):
    # Создаём флаг, который изначально равен одному какому-то значению
    flag = False
    for j in range(len(l2)):
        # Если находится подходящая пара
        if l1[i] + l2[j] == 100:
            # То надо как-то остановить поиск.
            # Иначе найденные числа затрутся следующими циклом
            # Для этого делаем break для внутреннего цикла и меняем значение флага для внешнего
            flag = True
            break
    
    # С помощью флага протаскиваем нашу команду break из внутреннего цикла во внешний
    if flag == True:
        break

print('Подходящая пара чисел', l1[i], l2[j])
